...
const CheckoutPage = () => {
  ...
  const total = items.reduce((sum: any, it: any) => sum + it.priceCents * it.qty, 0);

  async function submit() {
	setError(undefined);
	if (!items.length) { setError('Panier vide'); return; }
	if (!name || !email || !address) { setError('Veuillez remplir vos informations'); return; }
	setLoading(true);
	try {
	  const orderRes = await fetch(`${API_URL}/api/orders`, {
		method: 'POST', headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify({ userId: 'demo-user-1', items: items.map((i: any) => ({ sku: i.variantSKU, qty: i.qty })) })
	  });
  ...
  return (
	<div className="grid md:grid-cols-2 gap-6">
	  <div className="card space-y-3">
		...
		<textarea className="input h-28" placeholder="Adresse" value={address} onChange={e => setAddress(e.target.value)} />
		{error && <p className="text-red-600">{error}</p>}
		<button className="btn" disabled={loading} onClick={submit}>{loading ? 'Traitement…' : 'Payer (fictif)'}</button>
	  </div>
	  <div className="card">
		...
		<ul className="space-y-1 text-sm">
		  {items.map((it: any) => (
			<li key={it.variantSKU} className="flex justify-between">
			  <span>{it.variantSKU} × {it.qty}</span>
