...
const res = await fetch(`${API_URL}/api/products`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ name, brand, priceCents: price, category, images:['/img/placeholder1.jpg'] }) });
...
<input type="number" className="input" placeholder="Prix (cents)" value={price} onChange={e=>setPrice(Number(e.target.value))} />
...
<p className="text-xs text-gray-500">Les variantes/pointures peuvent être ajoutées côté back (seed) ou via un écran dédié.</p>
</div>
);
}